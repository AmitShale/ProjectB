...
class Simulation:
    def __init__(self, seed=1, num_robots=10, time_steps=1000, dt=0.1, space_limit=100, border_threshold=18, max_acc=8, max_vel=10, min_vel=0, max_steering_angle=30, run_away_steering_angle=40,
        ...
        # Statistics structs
        self.num_messages_arrived = np.zeros((self.num_robots, self.time_steps))
        self.communication_per_time_slot = np.zeros((self.num_robots, self.num_robots, self.time_steps), dtype=bool)
        ...
        self.positions_estimation = np.zeros((self.num_robots, 2, self.time_steps, self.time_steps))
    ...
    def update_positions(self):
        ...
        # Simulate measurement quality based on range
        for measuring_robot in range(self.num_robots):
            for measured_robot in range(self.num_robots):
                ...
                if self.no_erasures_communication:
                    if measuring_robot == measured_robot:
                        delay_cycles = 0
                    else:
                        delay_cycles = np.max([self.delay_std*np.random.randn()+self.delay_mean, 1])
    ...
    def predict_position(self):
        for i in range(self.num_robots):
            ...
            while self.measurement_lists[i]:
                measuring_robot, transmit_time, relative_arrival_cycles, trasnmit_pos, meas = self.measurement_lists[i][0]
            ...
            self.positions_estimation[i, :, :, self.t] = self.positions_estimation[i, :, :, self.t-1]    
